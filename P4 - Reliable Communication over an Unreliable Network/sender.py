...
def send_reliable(cs, filedata, receiver_binding, win_size):
    ...
    while(1 == 1):
        holder = win_left_edge
        win_left_edge = transmit_entire_window_from(win_left_edge)
        #print("Transmitted entire window")
        first_to_tx = win_left_edge
        print("first to tx is", + first_to_tx)
        while(last_acked < first_to_tx):   
            input = [cs]
            inputready, outputready, exceptready = select.select(input, [], [], RTO)
            if exceptready != []:
                print("Error")
            if inputready != []:
                s = inputready[0]
